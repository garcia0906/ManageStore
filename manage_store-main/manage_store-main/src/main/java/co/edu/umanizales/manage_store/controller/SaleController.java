...
public class SaleController {
    ...
    public ResponseEntity<ResponseDTO> getStoreByQuantity(int cant,String code) {
        int findSale = saleService.getTotalSales();
        while (findSale != 0) {

            return new ResponseEntity<>(new ResponseDTO(200, saleService.getTotalSalesByStore(code)>cant,null),HttpStatus.OK);

        }
        return new ResponseEntity<>(new ResponseDTO(409, "no hay ventas mayores a esa cantidad", null),
                HttpStatus.BAD_REQUEST);
